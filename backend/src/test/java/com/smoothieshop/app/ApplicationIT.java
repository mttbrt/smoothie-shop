...
public class ApplicationIT {
  ...
  public void getSmoothiesAsUser() throws Exception {
    MvcResult result = mvc.perform(get("/smoothies").with(csrf()))
        .andExpect(status().isOk()).andReturn();
    String resultAsStr = result.getResponse().getContentAsString();

    assertTrue(resultAsStr.contains("\"id\":1") && resultAsStr.contains("\"id\":2")
  ...
  public void getSmoothieByIdAsUser() throws Exception {
    MvcResult result = mvc.perform(get("/smoothies/1").with(csrf()))
        .andExpect(status().isOk()).andReturn();
    String resultAsStr = result.getResponse().getContentAsString();
  ...
  public void putSmoothieByIdAsUser() throws Exception {
    ...
    mvc.perform(put("/smoothies/1").with(csrf()).contentType(MediaType.APPLICATION_JSON_VALUE)
            .content(content))
  ...
  public void deleteSmoothieByIdAsUser() throws Exception {
    mvc.perform(delete("/smoothies/1").with(csrf()))
        .andExpect(status().isForbidden());
  }
  ...
  public void getSmoothieNutrientByIdAsUser() throws Exception {
    MvcResult result = mvc.perform(get("/smoothie-nutrients/1").with(csrf()))
        .andExpect(status().isOk()).andReturn();
    String resultAsStr = result.getResponse().getContentAsString();
  ...
  public void postOrderAsUser() throws Exception {
    ...
    mvc.perform(post("/orders").with(csrf()).contentType(MediaType.APPLICATION_JSON_VALUE)
            .content(content))
  ...
  public void putSmoothieByIdAsOwner() throws Exception {
    ...
    assertTrue(resultAsStr.contains("\"name\":\"test\",\"price\":5.9") && !resultAsStr.contains(
        "\"name\":\"Strawberry Galaxy\",\"price\":3.99"));
  }
  ...
  public void getSmoothieByNonExistingId() throws Exception {
    mvc.perform(get("/smoothies/10").with(csrf()))
  ...
  public void putSmoothieByIdWithMalformedData() throws Exception {
    ...
    mvc.perform(put("/smoothies/2").with(csrf()).contentType(MediaType.APPLICATION_JSON_VALUE)
            .content(content))
